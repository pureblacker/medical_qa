...
class LLMclientbase(object):
    def __init__(self):
        ...
        self.__client = OpenAI(
            api_key=os.environ.get("LLM_API_KEY" ),  # 使用环境变量中的API密钥初始化OpenAI客户端
            base_url=os.environ.get("LLM_BASE_URL"),  # 使用环境变量中的基础URL初始化OpenAI客户端
        )
        self.__model_name = os.environ.get("MODEL_NAME")  # 使用环境变量中的模型名称
        print('正在使用的模型是：',self.__client, self.__model_name)
    @property
    def client(self):

...
class Reranker:
    def __init__(self, use_fp16=True):
        ...
        # 获取基础目录和模型路径
        base_dir = os.getcwd()
        model_dir = load_nested_params("model","rerank","path")
        model_name = load_nested_params("model","rerank","name")
        # 拼接并规范化模型路径
        self.model_name = os.path.normpath(os.path.join(base_dir, model_dir, model_name))
        print('rerank model path:',self.model_name)
        # 加载分词器和模型
